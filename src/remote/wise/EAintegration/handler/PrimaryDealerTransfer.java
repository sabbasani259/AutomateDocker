...
import remote.wise.EAintegration.Qhandler.PrimaryDealerTransferQHandler;
...
{
	public PrimaryDealerTransfer()
	{

	}

	public PrimaryDealerTransfer(File processingFile, String archivedFolderPath, String process, String reprocessJobCode)
	{
		Logger fLogger = FatalLoggerClass.logger;
		try 
		{
			processPrimaryDealerTransfer(processingFile, archivedFolderPath, process, reprocessJobCode);
		} 
		catch (Exception e) 
		{
	...
	public void handlePrimaryDealerTransfer()
	{
		...
		try
		{
			Query query = session.createQuery("from JobDetails where jobCode='PrimaryDealerTransfer' order by sequence");
			...
			while(itr.hasNext())
			{
				...
				int depedencyLag = 0;
				if(jobDetailsObj.getProcess().equalsIgnoreCase("PrimaryDealerTransfer")){
					depedencyLag = new KafkaConsumerLag().getConsumerLag("DealerInfoQueue", "Client_DealerInfoQueue");
					while(depedencyLag > 0){
						iLogger.info("PrimaryDealerTransfer is in wait state because DealerInfo data was not yet processed. :"+depedencyLag);
	 ...
	 * @throws IOException
	 */
	public void processPrimaryDealerTransfer(File processingFile, String archivedFolderPath, String process, String reprocessJobCode)
	{
		Logger iLogger = InfoLoggerClass.logger;
		Logger fLogger = FatalLoggerClass.logger;		
		String queuePath = "jms/queue/PrimaryDealerTransferQueue";
		...
		try
		{
			...
			String processName="PrimaryDealerTransfer";
			while (line != null) 
			{
				...
				PrimaryDealerTransferInputContract inputObject = new PrimaryDealerTransferInputContract();
				...
				if(msgString.length>2)
					inputObject.setTransferDate(msgString[2]);
				...
				//Place the object into Primary Dealer Transfer Queue
				iLogger.info(inputObject.getSerialNumber()+": Q Object Details - Primary Dealer Transfer");
				iLogger.info(inputObject.getSerialNumber()+": --------------------------");
				iLogger.info(inputObject.getCustomerCode()+","+inputObject.getDealerCode()+"," +
						inputObject.getTransferDate()+"," +
						inputObject.getSerialNumber()+"," +
				...
				PrimaryDealerTransferQHandler queueObj = new PrimaryDealerTransferQHandler();
				//String qDropStatus = queueObj.handlePrimaryDealerTransfer(queuePath,inputObject);
				String qDropStatus = queueObj.handlePrimaryDealerTransferKafkaQueue("PrimaryDealerTransferQueue",inputObject);
			...
			if(runningNumber>1)
			{
				...
				try
				{
					...
					eaLogDetails.setProcess("PrimaryDealerTransfer");
			...
			//DF20160502 @Roopa calling PrimaryDealerTransferQHandler queue consumer
			/*PrimaryDealerTransferQHandler queueObj = new PrimaryDealerTransferQHandler();
