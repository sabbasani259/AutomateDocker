...
public class AssetGroup 
{
	...
	public AssetGroup(File processingFile, String archivedFolderPath, String process, String reprocessJobCode)
	{
		System.out.println("I am here");
		Logger fLogger = FatalLoggerClass.logger;
		try 
		{
			processAssetGroupData(processingFile, archivedFolderPath, process, reprocessJobCode);
		} 
		catch (Exception e) 
		{
	...
	public void handleAssetGroupDetails()
	{
		...
		try
		{
			Query query = session.createQuery("from JobDetails where jobCode='AssetGroup' order by sequence");
			...
			while(itr.hasNext())
			{
				...
				AssetGroup fileObj = new AssetGroup();
				...
				try
				{
					...
					for(int i=0; i<processingFiles.length; i++)
					{
						...
						fileObj.processAssetGroupData(processFile,archivedFolderPath, jobDetailsObj.getProcess(), jobDetailsObj.getReprocessJobCode());
					}
					...
					for (int j = 0; j < inputFiles.length; j++) 
					{
						...
						fileObj.processAssetGroupData(processingFile,archivedFolderPath, jobDetailsObj.getProcess(), jobDetailsObj.getReprocessJobCode());	       
					}
				}
	 ...
	 * @throws IOException
	 */
	public void processAssetGroupData(File processingFile, String archivedFolderPath, String process, String reprocessJobCode) 
	{
		Logger iLogger = InfoLoggerClass.logger;
		Logger fLogger = FatalLoggerClass.logger;

		String queuePath = "jms/queue/AssetGroupQueue";
		...
		try
		{
			...
			while (line != null) 
			{
				...
				try{
					...
					AssetGroupInputContract inputObject = new AssetGroupInputContract();

					if(msgString.length>0)
						inputObject.setAssetGroupName(msgString[0]);
					...
					iLogger.info(inputObject.getAssetGroupCode()+": Q Object Details - AssetGroupQueue");
					iLogger.info(inputObject.getAssetGroupCode()+": --------------------------");
					iLogger.info(inputObject.getAssetGroupName()+","+inputObject.getAssetGroupCode()+","+inputObject.getFileRef()+","+"" +
					...
					AssetGroupQHandler queueObj = new AssetGroupQHandler();
					//String qDropStatus = queueObj.handleAssetGroupDetails(queuePath,inputObject);
					String qDropStatus = queueObj.handleAssetGroupDetailsToKafkaQueue("AssetGroupQueue",inputObject);
			...
			if(runningNumber>1)
			{
				...
				try
				{
					...
					eaLogDetails.setProcess("AssetGroup");
			...
			//DF20160502 @Roopa calling AssetGroup queue consumer
			/*AssetGroupQHandler queueObj = new AssetGroupQHandler();
