...
public class AssetGroupQConsumer implements Runnable {

	Thread t;
	String topicName = "AssetGroupQueue";
	...
	public AssetGroupQConsumer() {
		t = new Thread(this, "AssetGroupQConsumer");
		t.start();
	}
	//CR330.sn
	@Override
	public void run() {
		...
		/*try {
			...
			fLogger.fatal("AssetGroupQConsumer:Error in intializing property File:" + e.getMessage());
		}*/
		...
		if (numberOfThreads != 0) {
			try {
				...
				ThreadFactory namedThreadFactory = new ThreadFactoryBuilder()
						.setNameFormat("AssetGroupQConsumer-thread-%d").build();
				List<TopicPartition> partitionList = new LinkedList<TopicPartition>();
				TopicPartition partition = new TopicPartition(topicName, 0);
				partitionList.add(partition);
				AssetGroupQConsumer.consumer = new KafkaConsumer<>(props);
				AssetGroupQConsumer.consumer.assign(partitionList);

				outerLoop: while (numberOfThreads != 0) {
					int msgsProcessed = 0;
					int ActiveCount = 0;

					if (AssetGroupQConsumer.listenerFlag == 1) {
						ConsumerRecords<String, String> streams = AssetGroupQConsumer.consumer.poll(100);
						...
						if (streams != null && !streams.isEmpty()) {
							...
							for (ConsumerRecord<String, String> stream : streams) {
								if (AssetGroupQConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"AssetGroupQConsumer:ForKafkaStreamStart:Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"AssetGroupQConsumer:ForKafkaStreamStart:Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									infiniteCounter = 0;
									while (AssetGroupQConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"AssetGroupQConsumer:ForKafkaStreamStart:Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
									...
									iLogger.info(
											"AssetGroupQConsumer:Control Came out of Infinite Loop when Lister Falg Set to 1");
									continue outerLoop;
								}
								...
								if (executor instanceof ThreadPoolExecutor) {
									...
									iLogger.debug("AssetGroupQConsumer: ActiveCount :" + ActiveCount);
								...
								if (AssetGroupQConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"AssetGroupQConsumer:It.hasNext():Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info("AssetGroupQConsumer:It.hasNext():Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									while (AssetGroupQConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info("AssetGroupQConsumer:It.hasNext():Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
								...
								msgsProcessed++;
								iLogger.debug("AssetGroupQConsumer:Number of Messages processed  =" + msgsProcessed);
								String msgReceived = stream.value();
								iLogger.info("AssetGroupQConsumer:ActiveCount : " + ActiveCount + " msgReceived ="
										+ msgReceived);

								AssetGroupQHandler assetGrpObj = new AssetGroupQHandler();
								assetGrpObj.assetGroupInput = msgReceived;

								Callable<String> worker = assetGrpObj;
							...
							iLogger.debug("AssetGroupQConsumer:Wait for Executor to terminate");
							int infiniteCounter1 = 0;
							while (!executor.isTerminated()) {
								if (infiniteCounter1 == 0) {
									iLogger.info(
											"AssetGroupQConsumer:It.When For loops Ended :Waiting for Termination");
									infiniteCounter1++;
								}
							}

							executor.shutdownNow();
							iLogger.debug("AssetGroupQConsumer:Executor terminated");
						}
						...
						fLogger.debug(
								"AssetGroupQConsumer:Listener Flag is set to 0. Hence not consuming messages from Topic");
						break outerLoop;
					}
				}
			}
				...
				fLogger.fatal("AssetGroupQConsumer:Unable to subscribe kafka consumer:Exception:" + e.getMessage());
			}

			finally {
				AssetGroupQConsumer.consumer.close();
				iLogger.info("AssetGroupQConsumer:End on Thread Run ");
			}
		}
	}
	//CR330.en
	...
	public void run() {
		...
		try {
			...
			fLogger.fatal("AssetGroupQConsumer:Error in fetching data property File:"+e.getMessage());
		}
			...
			try{
				...
				while(numberOfThreads!=0)
				{
					...
					if(AssetGroupQConsumer.listenerFlag==1)
					{
						...
						iLogger.info("AssetGroupQConsumer:Initialized with ConsumerGroupId:"+groupId);
	
						iLogger.info("AssetGroupQConsumer:Topic:"+topicName+":"+"Listener Flag is 1: Start Consumption from Topic"); 
							...
							for (KafkaStream stream : streams) {
								...
								while (consumerIterator.hasNext()){
									...
									if(msgsProcessed >= numberOfThreads){
										iLogger.info("AssetGroupQConsumer:ThreadPoolCount:"+numberOfThreads+":Total Messages processed in current run:"+msgsProcessed);
										iLogger.info("AssetGroupQConsumer:Wait for all the spawned threads to complete");
										executor.shutdown(); 
										while (!executor.isTerminated()) {   } 
										executor.shutdownNow();
										msgsProcessed=0;
									...
									iLogger.info("AssetGroupQConsumer:msgReceived ="+msgReceived);
	...
	//							iLogger.debug("AssetGroupQConsumer:Executor terminated"); 
				...
				fLogger.error("AssetGroupQConsumer: Cause of Exception :"+e.getMessage());
			}
		}
	}*/
	//CR330.eo
