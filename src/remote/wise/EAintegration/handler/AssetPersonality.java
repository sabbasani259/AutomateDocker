...
{
	public AssetPersonality()
	{

	}

	public AssetPersonality(File processingFile, String archivedFolderPath, String process, String reprocessJobCode)
	{
		Logger fLogger = FatalLoggerClass.logger;
		try 
		{
			processAssetPersonalityData(processingFile, archivedFolderPath, process, reprocessJobCode);
		} 
		catch (Exception e) 
		{
	...
	public void handleAssetPersonality()
	{
		...
		try
		{
			Query query = session.createQuery("from JobDetails where jobCode='AssetPersonality' order by sequence");
			...
			while(itr.hasNext())
			{
				...
				int depedencyLag = 0;
				try{
					if(jobDetailsObj.getProcess().equalsIgnoreCase("AssetPersonality")){
						depedencyLag = new KafkaConsumerLag().getConsumerLag("JcbRollOffQueue", "Client_JcbRollOffQueue");
						while(depedencyLag > 0){
							iLogger.info("AssetPersonality is wait state because RollOff data was not yet processed. :"+depedencyLag);
						...
						while(depedencyLag > 0){
							iLogger.info("AssetPersonality is wait state because AssetGroup data was not yet processed. :"+depedencyLag);
							TimeUnit.SECONDS.sleep(30);
							depedencyLag = new KafkaConsumerLag().getConsumerLag("AssetGroupQueue", "Client_AssetGroupQueue");
						}
						...
						while(depedencyLag > 0){
							iLogger.info("AssetPersonality is wait state because AssetType data was not yet processed. :"+depedencyLag);
							TimeUnit.SECONDS.sleep(30);
							depedencyLag = new KafkaConsumerLag().getConsumerLag("AssetTypeQueue", "Client_AssetTypeQueue");
						}
						...
						while(depedencyLag > 0){
							iLogger.info("AssetPersonality is wait state because EngineType data was not yet processed. :"+depedencyLag);
							TimeUnit.SECONDS.sleep(30);
							depedencyLag = new KafkaConsumerLag().getConsumerLag("EngineTypeQueue", "Client_EngineTypeQueue");
						}
					}
				...
				{
					e.printStackTrace();
					fLogger.fatal("AssetPersonality ::Exception in finding the depedencyLag:"+e.getMessage());
				}
	 ...
	 * @throws IOException
	 */
	public void processAssetPersonalityData(File processingFile, String archivedFolderPath, String process, String reprocessJobCode)
	{
		Logger iLogger = InfoLoggerClass.logger;
		Logger fLogger = FatalLoggerClass.logger;

		String queuePath = "jms/queue/AssetPersonalityQueue";
		...
		try
		{
			...
			String processName="AssetPersonality";
			...
			while (line != null) 
			{
				try{
					...
					AssetPersonalityInputContract inputObject = new AssetPersonalityInputContract();

					if(msgString.length>0)
						inputObject.setEngineNumber(msgString[0]);

					if(msgString.length>1)
						inputObject.setAssetGroupCode(msgString[1]);

					if(msgString.length>2)
						inputObject.setAssetTypeCode(msgString[2]);

					if(msgString.length>3)
						inputObject.setEngineTypeCode(msgString[3]);

					if(msgString.length>4)
						inputObject.setAssetBuiltDate(msgString[4]);

					if(msgString.length>5)
						inputObject.setMake(msgString[5]);

					if(msgString.length>6)
						inputObject.setFuelCapacity(msgString[6]);

					if(msgString.length>7)
						inputObject.setSerialNumber(msgString[7]);
					...
					iLogger.info(inputObject.getSerialNumber()+": Q Object Details - AssetPersonalityQueue");
					iLogger.info(inputObject.getSerialNumber()+": --------------------------");
					iLogger.info(inputObject.getEngineNumber()+","+inputObject.getAssetGroupCode()+"," +
							inputObject.getAssetTypeCode()+","+inputObject.getEngineTypeCode()+"," +
							inputObject.getAssetBuiltDate()+","+inputObject.getMake()+"," +
							inputObject.getFuelCapacity()+","+inputObject.getSerialNumber()+"," +
							inputObject.getFileRef()+","+"" +
							inputObject.getMessageId()+","+inputObject.getProcess()+","+inputObject.getReprocessJobCode());

					AssetPersonalityQHandler queueObj = new AssetPersonalityQHandler();
					//DF20171212:KO369761-Changing interfaces queues to Kafka from Hornet queues.
					//String qDropStatus = queueObj.handleAssetPersonalityDetails(queuePath,inputObject);
					String qDropStatus = queueObj.handleAssetPersonalityToKafkaQueue("AssetPersonalityQueue",inputObject);
					...
					if(runningNumber > 1){
						String machineNumber=msgString[7];
						if(runningNumber <= 51){
			...
			if(runningNumber>1)
			{
				...
				try
				{
					...
					eaLogDetails.setProcess("AssetPersonality");
					...
					//DF20180209 :@Maniratnam ## Interfaces backTracking--inserting into interface log details table total data count and successfully put to q count
					...
					iLogger.info("AssetPersonality ::Status on inserting data into interface log details table :"+iStatus);
