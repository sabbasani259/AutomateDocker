...
import remote.wise.service.implementation.MADREDAlertCountImpl;

import remote.wise.util.CommonUtil;
import remote.wise.util.DateUtil;
import remote.wise.util.ListToStringConversion;

@Path("/MADREDAlertRESTService")
public class MADREDAlertRESTService {

	@POST
	@Path("getAlertCount")
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes({MediaType.APPLICATION_JSON})
	public List<HashMap<String,String>> getAlertCount(@Context HttpHeaders httpHeaders,LinkedHashMap<String,Object> reqObj ) throws CustomFault
	{
		...
		try{
			...
			iLogger.info("MADREDAlertRESTService:getAlertCount ::  received csrftoken :: "+csrfToken);
			if(csrfToken!=null){
				isValidCSRF=util.validateANTICSRFTOKEN((String)reqObj.get("loginId"),csrfToken);
			}
			iLogger.info("MADREDAlertRESTService:getAlertCount ::   csrftoken isValidCSRF :: "+isValidCSRF);
			if(!isValidCSRF)
			{
				iLogger.info("MADREDAlertRESTService:getAlertCount ::  Invalid request.");
		...
		response = new MADREDAlertCountImpl().getRedAlertCount(accountIdStringList,filterListString,dateFilter,reqObj.get("downloadFlag").toString(),userId);
		long endTime=System.currentTimeMillis();
		iLogger.info("serviceName:MADREDAlertRESTService~executionTime:"+(endTime-startTime)+"~"+userId+"~");
