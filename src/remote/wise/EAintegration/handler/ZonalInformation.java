...
import remote.wise.EAintegration.Qhandler.ZonalInformationQHandler;
...
public class ZonalInformation 
{
	public ZonalInformation()
	{

	}

	public ZonalInformation(File processingFile, String archivedFolderPath, String process, String reprocessJobCode)
	{
		Logger fLogger = FatalLoggerClass.logger;
		try 
		{
			processZonalInformation(processingFile, archivedFolderPath, process, reprocessJobCode);
		} 
		catch (Exception e) 
		{
	...
	public void handleZonalInformation()
	{
		Logger fLogger = FatalLoggerClass.logger;
		...
		try
		{
			Query query = session.createQuery("from JobDetails where jobCode='ZonalInformation' order by sequence");
	 ...
	 * @throws IOException
	 */
	public void processZonalInformation(File processingFile, String archivedFolderPath, String process, String reprocessJobCode) 
	{
		Logger fLogger = FatalLoggerClass.logger;
		Logger iLogger = InfoLoggerClass.logger;

		String queuePath = "jms/queue/ZonalInformationQueue";
		...
		try
		{
			...
			while (line != null) 
			{
				...
				ZonalInformationInputContract inputObject = new ZonalInformationInputContract();

				if(msgString.length>0)
					inputObject.setZonalName(msgString[0]);

				if(msgString.length>1)
					inputObject.setZonalCode(msgString[1]);
				...
				iLogger.info(inputObject.getZonalName()+": Q Object Details - Zonal Information");
				iLogger.info(inputObject.getZonalName()+": --------------------------");
				iLogger.info(inputObject.getZonalName()+","+inputObject.getZonalCode()+"," +
						inputObject.getFileRef()+","+"" +
						inputObject.getMessageId()+","+inputObject.getProcess()+","+inputObject.getReprocessJobCode());

				ZonalInformationQHandler queueObj = new ZonalInformationQHandler();
				//DF20171212:KO369761-Changing interfaces queues to Kafka from Hornet queues.
				//String qDropStatus = queueObj.handleZonalInformation(queuePath,inputObject);
				String qDropStatus = queueObj.handleZonalInformationToKafkaQueue("ZonalInformationQueue",inputObject);
			...
			//DF20160502 @Roopa calling ZonalInformationQHandler queue consumer
