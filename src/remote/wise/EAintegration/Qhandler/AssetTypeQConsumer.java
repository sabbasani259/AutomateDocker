...
public class AssetTypeQConsumer implements Runnable {

	Thread t;
	String topicName = "AssetTypeQueue";
	...
	public AssetTypeQConsumer() {
		t = new Thread(this, "AssetTypeQConsumer");
		t.start();
	}
	...
	public void run() {
		...
		/*try {
			...
			numberOfThreads = Integer.valueOf(prop.getProperty("AssetTypeQThreadCount"));
		} catch (Exception e) {
			fLogger.fatal("AssetTypeQConsumer:Error in intializing property File:" + e.getMessage());
		}*/
		...
		if (numberOfThreads != 0) {
			try {
				...
				ThreadFactory namedThreadFactory = new ThreadFactoryBuilder()
						.setNameFormat("AssetTypeQConsumer-thread-%d").build();
				List<TopicPartition> partitionList = new LinkedList<TopicPartition>();
				TopicPartition partition = new TopicPartition(topicName, 0);
				partitionList.add(partition);
				AssetTypeQConsumer.consumer = new KafkaConsumer<>(props);
				AssetTypeQConsumer.consumer.assign(partitionList);

				outerLoop: while (numberOfThreads != 0) {
					int msgsProcessed = 0;
					int ActiveCount = 0;

					if (AssetTypeQConsumer.listenerFlag == 1) {
						ConsumerRecords<String, String> streams = AssetTypeQConsumer.consumer.poll(100);
						...
						if (streams != null && !streams.isEmpty()) {
							...
							for (ConsumerRecord<String, String> stream : streams) {
								if (AssetTypeQConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"AssetTypeQConsumer:ForKafkaStreamStart:Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"AssetTypeQConsumer:ForKafkaStreamStart:Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									infiniteCounter = 0;
									while (AssetTypeQConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info("AssetTypeQConsumer:ForKafkaStreamStart:Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
									...
									iLogger.info(
											"AssetTypeQConsumer:Control Came out of Infinite Loop when Lister Falg Set to 1");
									continue outerLoop;
								}
								...
								if (executor instanceof ThreadPoolExecutor) {
									...
									iLogger.debug("AssetTypeQConsumer: ActiveCount :" + ActiveCount);
								...
								if (AssetTypeQConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"AssetTypeQConsumer:It.hasNext():Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info("AssetTypeQConsumer:It.hasNext():Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									while (AssetTypeQConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info("AssetTypeQConsumer:It.hasNext():Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
								...
								msgsProcessed++;
								iLogger.debug("AssetTypeQConsumer:Number of Messages processed  =" + msgsProcessed);
								String msgReceived = stream.value();
								iLogger.info("AssetTypeQConsumer:ActiveCount : " + ActiveCount + " msgReceived ="
										+ msgReceived);

								AssetTypeQHandler assetTypeObj = new AssetTypeQHandler();
								assetTypeObj.assetTypeInput = msgReceived;

								Callable<String> worker = assetTypeObj;
							...
							iLogger.debug("AssetTypeQConsumer:Wait for Executor to terminate");
							int infiniteCounter1 = 0;
							while (!executor.isTerminated()) {
								if (infiniteCounter1 == 0) {
									iLogger.info("AssetTypeQConsumer:It.When For loops Ended :Waiting for Termination");
									infiniteCounter1++;
								}
							}

							executor.shutdownNow();
							iLogger.debug("AssetTypeQConsumer:Executor terminated");
						}
						...
						fLogger.debug(
								"AssetTypeQConsumer:Listener Flag is set to 0. Hence not consuming messages from Topic");
						break outerLoop;
					}
				}
			}
				...
				fLogger.fatal("AssetTypeQConsumer:Unable to subscribe kafka consumer:Exception:" + e.getMessage());
			}

			finally {
				AssetTypeQConsumer.consumer.close();
				iLogger.info("AssetTypeQConsumer:End on Thread Run ");
			}
		}
	}
	//CR330.en
	...
	public void run() {
		...
		try {
			...
			fLogger.fatal("AssetTypeQConsumer:Error in fetching data property File:"+e.getMessage());
		}
			...
			try{
				...
				while(numberOfThreads!=0)
				{
					...
					if(AssetTypeQConsumer.listenerFlag==1)
					{
						...
						iLogger.info("AssetTypeQConsumer:Initialized with ConsumerGroupId:"+groupId);
	
						iLogger.info("AssetTypeQConsumer:Topic:"+topicName+":"+"Listener Flag is 1: Start Consumption from Topic"); 
							...
							for (KafkaStream stream : streams) {
								...
								while (consumerIterator.hasNext()){
									...
									if(msgsProcessed >= numberOfThreads){
										iLogger.info("AssetTypeQConsumer:ThreadPoolCount:"+numberOfThreads+":Total Messages processed in current run:"+msgsProcessed);
										iLogger.info("AssetTypeQConsumer:Wait for all the spawned threads to complete");
										executor.shutdown(); 
										while (!executor.isTerminated()) {   } 
										executor.shutdownNow();
										msgsProcessed=0;
									...
									iLogger.info("AssetTypeQConsumer:msgReceived ="+msgReceived);
	...
	//							iLogger.debug("AssetTypeQConsumer:Executor terminated"); 
				...
				fLogger.error("AssetTypeQConsumer: Cause of Exception :"+e.getMessage());
			}
		}
	}*/
	//CR330.eo
