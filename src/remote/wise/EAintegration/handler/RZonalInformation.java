...
public class RZonalInformation 
{
	public void reprocessZonalInformation()
	{
      ...
      if(session.getTransaction().isActive() && session.isDirty())
      {
        ...
        try
        {
        	...
        	List<ZonalInformationInputContract> inputContractList = new LinkedList<ZonalInformationInputContract>();
        	...
        	Query query = session.createQuery("select a.messageId, a.messageString, a.fileName, a.process," +
        			...
        			" a.process = b.process and a.reprocessJobCode = 'RZonalInformation' and a.reprocessTimeStamp <= '"+currentTimestamp+"' " +
        	...
        	while(itr.hasNext())
        	{
        		...
        		if(paramSize>0)
        			zonalName = messageSplit[0];
        		if(paramSize>1)
        			zonalCode = messageSplit[1];
        		        		        		
        		//Fill the input contract Object
        		ZonalInformationInputContract inputContractObj = new ZonalInformationInputContract();
        		inputContractObj.setZonalName(zonalName);
        		inputContractObj.setZonalCode(zonalCode);
        	...
        	{
        		ZonalInformationClient clientObj = new ZonalInformationClient();
        		response = clientObj.invokeZonalInformation(inputContractList.get(i));
