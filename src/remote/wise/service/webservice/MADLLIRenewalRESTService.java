...
import remote.wise.service.implementation.MADLLIRenewalImpl;
...
@Path("/MADLLIRenewalRESTService")
public class MADLLIRenewalRESTService {

	@POST
	@Path("getRenewedMachine")
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes({MediaType.APPLICATION_JSON})
	public List<HashMap<String,String>> getRenewedMachine(@Context HttpHeaders httpHeaders,LinkedHashMap<String,Object> reqObj ) throws CustomFault
	{
		...
		try{
			...
			iLogger.info("MADLLIRenewalRESTService:getRenewedMachine ::  received csrftoken :: "+csrfToken);
			if(csrfToken!=null){
				isValidCSRF=util.validateANTICSRFTOKEN((String)reqObj.get("loginId"),csrfToken);
			}
			iLogger.info("MADLLIRenewalRESTService:getRenewedMachine ::   csrftoken isValidCSRF :: "+isValidCSRF);
			if(!isValidCSRF)
			{
				iLogger.info("MADLLIRenewalRESTService:getRenewedMachine ::  Invalid request.");
			...
			for(int i=0;i<reqObj.size();i++)
			{
				...
				if(reqObj.get("filterList")!=null){
					...
					filterListString=convert.getStringList(filterList).toString();
				}
		...
		response = new MADLLIRenewalImpl().getRenewalCount(accountIdStringList,filterListString,dateFilter,reqObj.get("downloadFlag").toString(),userId);
		long endTime=System.currentTimeMillis();
		iLogger.info("serviceName:MADLLIRenewalRESTService~executionTime:"+(endTime-startTime)+"~"+userId+"~");
