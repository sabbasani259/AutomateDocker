...
public class JCBRollOffQConsumer implements Runnable {

	Thread t;
	String topicName = "JcbRollOffQueue";
	...
	public JCBRollOffQConsumer() {
		t = new Thread(this, "JCBRollOffQConsumer");
		t.start();
	}
	...
	public void run() {
		...
		/*try {
			...
			numberOfThreads = Integer.valueOf(prop.getProperty("JCBRollOffQThreadCount"));
		} catch (Exception e) {
			fLogger.fatal("JCBRollOffQConsumer:Error in intializing property File:" + e.getMessage());
		}*/
		...
		if (numberOfThreads != 0) {
			try {
				...
				ThreadFactory namedThreadFactory = new ThreadFactoryBuilder().setNameFormat("JCBRollOffQConsumer-thread-%d")
						.build();
				List<TopicPartition> partitionList = new LinkedList<TopicPartition>();
				TopicPartition partition = new TopicPartition(topicName, 0);
				partitionList.add(partition);
				JCBRollOffQConsumer.consumer = new KafkaConsumer<>(props);
				JCBRollOffQConsumer.consumer.assign(partitionList);

				outerLoop: while (numberOfThreads != 0) {
					int msgsProcessed = 0;
					int ActiveCount = 0;

					if (JCBRollOffQConsumer.listenerFlag == 1) {
						ConsumerRecords<String, String> streams = JCBRollOffQConsumer.consumer.poll(100);
						...
						if (streams != null && !streams.isEmpty()) {
							...
							for (ConsumerRecord<String, String> stream : streams) {
								if (JCBRollOffQConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"JCBRollOffQConsumer:ForKafkaStreamStart:Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"JCBRollOffQConsumer:ForKafkaStreamStart:Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									infiniteCounter = 0;
									while (JCBRollOffQConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"JCBRollOffQConsumer:ForKafkaStreamStart:Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
									...
									iLogger.info(
											"JCBRollOffQConsumer:Control Came out of Infinite Loop when Lister Falg Set to 1");
									continue outerLoop;
								}
								...
								if (executor instanceof ThreadPoolExecutor) {
									...
									iLogger.debug("JCBRollOffQConsumer: ActiveCount :" + ActiveCount);
								...
								if (JCBRollOffQConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"JCBRollOffQConsumer:It.hasNext():Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"JCBRollOffQConsumer:It.hasNext():Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									while (JCBRollOffQConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info("JCBRollOffQConsumer:It.hasNext():Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
								...
								msgsProcessed++;
								iLogger.debug(
										"JCBRollOffQConsumer:Number of Messages processed  =" + msgsProcessed);
								String msgReceived = stream.value();
								iLogger.info("JCBRollOffQConsumer:ActiveCount : " + ActiveCount + " msgReceived ="
										+ msgReceived);

								JcbRollOffQHandler rollOffObj = new JcbRollOffQHandler();
								rollOffObj.JCBRollOffInput = msgReceived;

								Callable<String> worker = rollOffObj;
							...
							iLogger.debug("JCBRollOffQConsumer:Wait for Executor to terminate");
							int infiniteCounter1 = 0;
							while (!executor.isTerminated()) {
								if (infiniteCounter1 == 0) {
									iLogger.info(
											"JCBRollOffQConsumer:It.When For loops Ended :Waiting for Termination");
									infiniteCounter1++;
								}
							}

							executor.shutdownNow();
							iLogger.debug("JCBRollOffQConsumer:Executor terminated");
						}
						...
						fLogger.debug(
								"JCBRollOffQConsumer:Listener Flag is set to 0. Hence not consuming messages from Topic");
						break outerLoop;
					}
				}
			}
				...
				fLogger.fatal("JCBRollOffQConsumer:Unable to subscribe kafka consumer:Exception:" + e.getMessage());
			}

			finally {
				JCBRollOffQConsumer.consumer.close();
				iLogger.info("JCBRollOffQConsumer:End on Thread Run ");
			}
		}
	}
	//CR330.en
	...
	public void run() {
		...
		try {
			...
			fLogger.fatal("JCBRollOffQConsumer:Error in fetching data property File:"+e.getMessage());
		}
			...
			try{
				...
				while(numberOfThreads!=0)
				{
					...
					if(JCBRollOffQConsumer.listenerFlag==1)
					{
						...
						iLogger.info("JCBRollOffQConsumer:Initialized with ConsumerGroupId:"+groupId);
	
						iLogger.info("JCBRollOffQConsumer:Topic:"+topicName+":"+"Listener Flag is 1: Start Consumption from Topic"); 
							...
							for (KafkaStream stream : streams) {
								...
								while (consumerIterator.hasNext()){
									...
									if(msgsProcessed >= numberOfThreads){
										iLogger.info("JCBRollOffQConsumer:ThreadPoolCount:"+numberOfThreads+":Total Messages processed in current run:"+msgsProcessed);
										iLogger.info("JCBRollOffQConsumer:Wait for all the spawned threads to complete");
										executor.shutdown(); 
										while (!executor.isTerminated()) {   } 
										executor.shutdownNow();
										msgsProcessed=0;
									...
									iLogger.info("JCBRollOffQConsumer:msgReceived ="+msgReceived);
	...
	//							iLogger.debug("JCBRollOffQConsumer:Executor terminated"); 
				...
				fLogger.error("JCBRollOffQConsumer: Cause of Exception :"+e.getMessage());
			}
		}
	}*/
	//CR330.eo
