 ...
 *
 */
@WebService(name = "LandmarkDetailsService")
public class LandmarkDetailsService {
	
	/**
	 *This method gets all landmark details that belongs specified Tenancy ID and Login Id
	 * @param reqObj  Get all landmark details for a given Tenancy ID
	 * @return respObj Returns list of landmark details attached for the Tenancy ID  defined.
	 * @throws CustomFault
	 */
	@WebMethod(operationName = "GetLandmarkDetails", action = "GetLandmarkDetails")
	public List<LandmarkDetailsRespContract> getLandmarkDetails(@WebParam(name="ReqObj") LandmarkDetailsReqContract reqObj) throws CustomFault{

		//DefectId:1337 - Rajani Nagaraju - 20130923 - Log4j Changes - Using static logger object all throughout the application
		//WiseLogger infoLogger = WiseLogger.getLogger("LandmarkDetailsService:","info");
		...
		iLogger.info("login_id:"+reqObj.getLogin_id()+","+"Tenancy_ID:"+reqObj.getTenancy_ID()+","+"Landmark_id:"+reqObj.getLandmark_id()+"");
		...
		if(csrfToken != null){
			...
			iLogger.info("LandmarkDetails :: getLandmarkDetails ::   csrftoken isValidCSRF :: "
					+ isValidCSRF);
		}
		if (!isValidCSRF) {
		...
		List<LandmarkDetailsRespContract> respObj = new LandmarkDetailsImpl().getLandmarkDetails(reqObj);
		iLogger.info("----- Webservice Output-----");
		for(int i=0; i<respObj.size(); i++){
			iLogger.info("Landmark_id:"+respObj.get(i).getLandmark_id()+","+"Landmark_Category_ID:"+respObj.get(i).getLandmark_Category_ID()+","+"Landmark_Name:"+respObj.get(i).getLandmark_Name()+","+"Latitude:"+respObj.get(i).getLatitude()+","+"Longitude:"+respObj.get(i).getLongitude()+","+"Radius:"+respObj.get(i).getRadius()+","+"Address:"+respObj.get(i).getAddress()+","+"IsArrival:"+respObj.get(i).getIsArrival()+","+"IsDeparture:"+respObj.get(i).getIsDeparture()+",");
	...
	/**
	 * This method sets all landmark details that belongs specified Landmark Category ID and Landmark Name
	 * @param setLandmarkDetails:Get Landmark and set Landmark Details to the corresponding to it
	...
	public LandmarkDetailsReqContract setLandmarkDetails(@WebParam(name = "RespObj")LandmarkDetailsRespContract RespObj) throws CustomFault
	{ 
		...
		long startTime = System.currentTimeMillis();
		iLogger.info("---- Webservice Input ------");
		iLogger.info("Landmark_id:"+RespObj.getLandmark_id()+","+"Landmark_Category_ID:"+RespObj.getLandmark_Category_ID()+","+"Landmark_Name:"+RespObj.getLandmark_Name()+","+"Latitude:"+RespObj.getLatitude()+","+"Longitude:"+RespObj.getLongitude()+","+"Radius:"+RespObj.getRadius()+","+"Address:"+RespObj.getAddress()+","+"IsArrival:"+RespObj.getIsArrival()+","+"IsDeparture:"+RespObj.getIsDeparture()+",");
		...
		if (csrfToken != null) {
			isValidCSRF = util.validateANTICSRFTOKEN(loginId, csrfToken);
			iLogger.info("setLandmarkDetails ::   csrftoken isValidCSRF :: "
					+ isValidCSRF);
			if (!isValidCSRF) {
