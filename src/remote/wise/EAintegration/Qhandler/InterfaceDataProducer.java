...
public class InterfaceDataProducer implements Runnable{

	Thread t;
	String txnKey;
	HashMap<String,String> payloadMap;

	public InterfaceDataProducer()
	{

	}
	public InterfaceDataProducer(String txnKey,HashMap<String,String> payloadMap)
	{
	...
	public String publishMachineData(String txnKey,HashMap<String,String> payloadMap)
	{
		...
		String topicName = prop.getProperty("InterfaceKafaTopic");
		try
		{
	...
	{	
    	...
    	iLogger.info(txnKey+":InterfaceDataProducer ::KafkaPublisher:"+message +": topicName :"+topicName+" Put To Kafka Producer : START");
    	...
    	try{
    		...
    		long endTime=System.currentTimeMillis();
    		iLogger.info("InterfaceDataProducer :: publishMessage to KafkaPublisher data:"+message+":" +"publishMessage to KafkaPublisher : END :"+(endTime-startTime)); 
    	}catch (Exception e) {
    		e.printStackTrace();
			status="FAILURE";
			fLogger.fatal(txnKey+"InterfaceDataProducer :KafkaPublisher:"+"Exception in publishing the message to Topic:"+topicName+"; Persisting payload in OrientDB:"+e.getMessage());	
    	}
		return status;
	}
