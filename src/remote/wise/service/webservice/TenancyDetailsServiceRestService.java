 ...
 */
@Path("TenancyDetailsServiceRestService")
public class TenancyDetailsServiceRestService
{
	 ...
	 */
	@POST
	@Path( "setTenancyDetails")
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces(MediaType.APPLICATION_JSON)
	public String setTenancyDetails(@WebParam(name="reqObj" ) TenancyCreationReqContract reqObj) 
	{
		...
		if(!utilObj.roleValidationAgainstLoginId(UserID, reqObj.getTenancyAdminRoleId())){
		 ...
		 response = new TenancyDetailsImpl().createTenancy(reqObj);
	 ...
	 */
	@POST
	@Path( "GetTenancyDetails")
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces(MediaType.APPLICATION_JSON) 
	public List<TenancyDetailsRespContract> getTenancyDetails(@WebParam(name="reqObj" ) TenancyDetailsReqContract reqObj) 
	{
		//DefectId:1337 - Rajani Nagaraju - 20130923 - Log4j Changes - Using static logger object all throughout the application
		//WiseLogger infoLogger = WiseLogger.getLogger("TenancyDetailsService:","info");
		Logger iLogger = InfoLoggerClass.logger;
		...
		int pageNumber = 1;
		String page = null;
		List<TenancyDetailsRespContract> response=null;
		try {
		...
		reqObj.setLoginId(UserID);
		iLogger.info("Decoded userId::"+reqObj.getLoginId());
		 response = new TenancyDetailsImpl().getTenancyDetails(reqObj,pageNumber);
		iLogger.info("----- Webservice Output-----");
