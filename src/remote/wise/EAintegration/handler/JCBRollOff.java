...
public class JCBRollOff 
{
	...
	//Constructor that invokes the method to process EA data
	public JCBRollOff(File processingFile, String archivedFolderPath, String process, String reprocessJobCode)
	{
		Logger fLogger = FatalLoggerClass.logger;
		try 
		{
			processJcbRollOffData(processingFile, archivedFolderPath, process, reprocessJobCode);
		} 
		catch (Exception e) 
		{
	...
	public void handleJcbRollOffEAdata()
	{
		...
		{
			iLogger.info("JCBRollOff - "+"Opening a new session");
		...
		try
		{
			Query query = session.createQuery("from JobDetails where jobCode='JCBRollOff' order by sequence");
			...
			while(itr.hasNext())
			{
				...
				JCBRollOff fileObj = new JCBRollOff();
				...
				//------------------------- Get the files related to JcbRollOff
				...
				try
				{
					...
					for(int i=0; i<processingFiles.length; i++)
					{
						...
						iLogger.info("JCBRollOff - "+"File to be processed:"+processFile.getName());
						fileObj.processJcbRollOffData(processFile,archivedFolderPath, jobDetailsObj.getProcess(), jobDetailsObj.getReprocessJobCode());
					}
					...
					for (int j = 0; j < inputFiles.length; j++) 
					{
						...
						if(fileMoveStatus)
							iLogger.info("JCBRollOff - "+"File "+inputFile.getName()+" is moved successfully from Input to processing folder");

						fileObj.processJcbRollOffData(processingFile,archivedFolderPath, jobDetailsObj.getProcess(), jobDetailsObj.getReprocessJobCode());	       
					}
				}
	 ...
	 * @throws IOException
	 */
	public void processJcbRollOffData(File processingFile, String archivedFolderPath, String process, String reprocessJobCode)
	{
		Logger iLogger = InfoLoggerClass.logger;
		Logger fLogger = FatalLoggerClass.logger;

		String queuePath = "jms/queue/JcbRollOffQueue";
		...
		if(session.getTransaction().isActive() && session.isDirty())
		{
			iLogger.info("JCBRollOff - processJcbRollOffData "+"Opening a new session");
			session = HibernateUtil.getSessionFactory().openSession();
		}
		session.beginTransaction();
		...
		try
		{
			...
			if(duplicateFile==1)
			{
				...
				iLogger.info("JCBRollOff - "+"File "+processingFile.getName()+" is already processed and hence skipping the same");
				...
				if(moveStatus)
					iLogger.info("JCBRollOff - "+"File "+processingFile.getName()+" is moved successfully from processing to archived folder");
		...
		{
			fLogger.fatal("JCBRollOff - "+processingFile.getName()+": Exception :"+e);
		}
		...
		finally
		{
			...
			{
				fLogger.fatal("JCBRollOff - "+"Exception in commiting the record:"+e2);
		...
		iLogger.info("JCBRollOff - processJcbRollOffData "+"Closing a  session");
		...
		try
		{
			...
			//Shajesh :25-09-2020 : if Filename length is >60 , truncate the file name and save it to the corresponding table
			...
			while (line != null) 
			{
				try{
					...
					JcbRollOffInputContract inputObject = new JcbRollOffInputContract();

					if(msgString.length>0)
						inputObject.setSerialNumber(msgString[0]);

					if(msgString.length>2)
						inputObject.setChasisNumber(msgString[2]);

					if(msgString.length>1)
						inputObject.setEngineNumber(msgString[1]);

					if(msgString.length>3)
						inputObject.setMake(msgString[3]);

					if(msgString.length>4)
						inputObject.setBuiltDate(msgString[4]);

					if(msgString.length>5)
						inputObject.setMachineNumber(msgString[5]);
					...
					//Place the object into JcbRollOffQ
					iLogger.info("JCBRollOff - "+inputObject.getSerialNumber()+": Q Object Details");
					iLogger.info(inputObject.getSerialNumber()+" :--------------------------");
					iLogger.info("JCBRollOff- processJcbRollOffData Input to the Q::"+inputObject.getChasisNumber()+","+inputObject.getEngineNumber()+","+inputObject.getFileRef()+","+"" +
							inputObject.getMessageId()+","+inputObject.getProcess()+","+inputObject.getReprocessJobCode()+","+inputObject.getSerialNumber()+","+"" +
							inputObject.getMake()+","+inputObject.getBuiltDate()+","+inputObject.getMachineNumber());

					JcbRollOffQHandler queueObj = new JcbRollOffQHandler();
					iLogger.info("JCBRollOff - processJcbRollOffData Before Calling JCBRollOff Q Handler");
					...
					String qDropStatus = queueObj.handleJcbRollOffToKafkaQueue("JcbRollOffQueue",inputObject);

					iLogger.info("JCBRollOff - processJcbRollOffData After Calling JCBRollOff Q Handler Status::"+qDropStatus);
					...
					if(runningNumber > 1){
						String machineNumber=msgString[5];
						if(runningNumber <= 51){
			...
			if(runningNumber>1)
			{
				...
				if(session.getTransaction().isActive() && session.isDirty())
				{
					iLogger.info("JCBRollOff - processJcbRollOffData "+"Opening a new session to insert the record into EALogDetails");
					session = HibernateUtil.getSessionFactory().openSession();
				}
				...
				try
				{
					...
					eaLogDetails.setProcess("RollOff");
				...
				iLogger.info("JCBRollOff - processJcbRollOffData "+"Closed a  session to insert the record into EALogDetails");
			}
			...
			//DF20160502 @Roopa calling JcbRollOffQHandler queue consumer
			/*JcbRollOffQHandler queueObj = new JcbRollOffQHandler();
