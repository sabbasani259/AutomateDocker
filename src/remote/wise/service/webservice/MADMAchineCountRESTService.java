...
@Path("/MADMAchineCountRESTService")
public class MADMAchineCountRESTService {

	@POST
	@Path("getMachineCount")
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes({MediaType.APPLICATION_JSON})
	public List<HashMap<String,String>> getMachineCount(@Context HttpHeaders httpHeaders,LinkedHashMap<String,Object> reqObj ) throws CustomFault
	{
		...
		try{
			...
			iLogger.info("MADMAchineCountRESTService:getMachineCount ::  received csrftoken :: "+csrfToken);
			if(csrfToken!=null){
				isValidCSRF=util.validateANTICSRFTOKEN((String)reqObj.get("loginId"),csrfToken);
			}
			iLogger.info("MADMAchineCountRESTService:getMachineCount ::   csrftoken isValidCSRF :: "+isValidCSRF);
			if(!isValidCSRF)
			{
				iLogger.info("MADMAchineCountRESTService:getMachineCount ::  Invalid request.");
		...
		response = new MADMAchineCountImpl().getMachineCount(accountIdStringList,filterListString,dateFilter,reqObj.get("downloadFlag").toString(),userId);
		long endTime=System.currentTimeMillis();
		iLogger.info("serviceName:MADMAchineCountRESTService~executionTime:"+(endTime-startTime)+"~"+userId+"~");
	}
	catch(Exception e){
		e.printStackTrace();
