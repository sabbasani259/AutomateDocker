...
import remote.wise.EAintegration.Qhandler.DealerMappingQHandler;
...
public class DealerMapping 
{
	...
	public DealerMapping(File processingFile, String archivedFolderPath, String process, String reprocessJobCode)
	{
		Logger fLogger = FatalLoggerClass.logger;
		try 
		{
			processDealerMappingData(processingFile, archivedFolderPath, process, reprocessJobCode);
		} 
		catch (Exception e) 
		{
	...
	public void handleDealerMappingDetails()
	{
		...
		try
		{
			Query query = session.createQuery("from JobDetails where jobCode='DealerMapping' order by sequence");
			...
			while(itr.hasNext())
			{
				...
				DealerMapping fileObj = new DealerMapping();
				...
				//------------------------- Get the files related to AssetGroup
				...
				try
				{
					...
					for(int i=0; i<processingFiles.length; i++)
					{
						...
						fileObj.processDealerMappingData(processFile,archivedFolderPath, jobDetailsObj.getProcess(), jobDetailsObj.getReprocessJobCode());
					}
					...
					for (int j = 0; j < inputFiles.length; j++) 
					{
						...
						fileObj.processDealerMappingData(processingFile,archivedFolderPath, jobDetailsObj.getProcess(), jobDetailsObj.getReprocessJobCode());	       
					}
				}
	 ...
	 * @throws IOException
	 */
	public void processDealerMappingData(File processingFile, String archivedFolderPath, String process, String reprocessJobCode) 
	{
		Logger iLogger = InfoLoggerClass.logger;
		Logger fLogger = FatalLoggerClass.logger;

		String queuePath = "jms/queue/DealerMappingQueue";
		...
		try
		{
			...
			while (line != null) 
			{
				...
				DealerMappingInputContract inputObject = new DealerMappingInputContract();

				if(msgString.length>0)
					inputObject.setEccCode(msgString[0]);

				if(msgString.length>1)
					inputObject.setCrmCode(msgString[1]);

				if(msgString.length>2)
					inputObject.setLlCode(msgString[2]);

				if(msgString.length>3)
					inputObject.setDealerName(msgString[3]);
				...
				iLogger.info(inputObject.getDealerName()+": Q Object Details - DealerMappingQueue");
				iLogger.info(inputObject.getDealerName()+" --------------------------");
				iLogger.info(inputObject.getEccCode()+","+inputObject.getCrmCode()+","+
						inputObject.getLlCode()+","+inputObject.getDealerName()+
						inputObject.getFileRef()+","+"" +
						inputObject.getMessageId()+","+inputObject.getProcess()+","+inputObject.getReprocessJobCode());

				DealerMappingQHandler queueObj = new DealerMappingQHandler();
				//DF20171212:KO369761-Changing interfaces queues to Kafka from Hornet queues.
				//String qDropStatus = queueObj.handleDealerMappingDetails(queuePath,inputObject);
				String qDropStatus = queueObj.handleDealerMappingToKafkaQueue("DealerMappingQueue",inputObject);
			...
			if(runningNumber>1)
			{
				...
				try
				{
					...
					eaLogDetails.setProcess("DealerMapping");
			...
			//DF20160428 @Roopa calling DealerMapping queue consumer
			/*DealerMappingQHandler queueObj = new DealerMappingQHandler();
