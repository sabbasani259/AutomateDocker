...
public class SaleFromD2CConsumer implements Runnable {

	Thread t;
	String topicName = "SaleFromD2CQueue";
	...
	public SaleFromD2CConsumer() {
		t = new Thread(this, "SaleFromD2CConsumer");
		t.start();
	}
	...
	public void run() {
		...
		/*try {
			...
			numberOfThreads = Integer.valueOf(prop.getProperty("SaleFromD2CQThreadCount"));
		} catch (Exception e) {
			fLogger.fatal("SaleFromD2CConsumer:Error in intializing property File:" + e.getMessage());
		}*/
		...
		if (numberOfThreads != 0) {
			try {
				...
				ThreadFactory namedThreadFactory = new ThreadFactoryBuilder()
						.setNameFormat("SaleFromD2CConsumer-thread-%d").build();
				List<TopicPartition> partitionList = new LinkedList<TopicPartition>();
				TopicPartition partition = new TopicPartition(topicName, 0);
				partitionList.add(partition);
				SaleFromD2CConsumer.consumer = new KafkaConsumer<>(props);
				SaleFromD2CConsumer.consumer.assign(partitionList);

				outerLoop: while (numberOfThreads != 0) {
					int msgsProcessed = 0;
					int ActiveCount = 0;

					if (SaleFromD2CConsumer.listenerFlag == 1) {
						ConsumerRecords<String, String> streams = SaleFromD2CConsumer.consumer.poll(100);
						...
						if (streams != null && !streams.isEmpty()) {
							...
							for (ConsumerRecord<String, String> stream : streams) {
								if (SaleFromD2CConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"SaleFromD2CConsumer:ForKafkaStreamStart:Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"SaleFromD2CConsumer:ForKafkaStreamStart:Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									infiniteCounter = 0;
									while (SaleFromD2CConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"SaleFromD2CConsumer:ForKafkaStreamStart:Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
									...
									iLogger.info(
											"SaleFromD2CConsumer:Control Came out of Infinite Loop when Lister Falg Set to 1");
									continue outerLoop;
								}
								...
								if (executor instanceof ThreadPoolExecutor) {
									...
									iLogger.debug("SaleFromD2CConsumer: ActiveCount :" + ActiveCount);
								...
								if (SaleFromD2CConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"SaleFromD2CConsumer:It.hasNext():Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"SaleFromD2CConsumer:It.hasNext():Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									while (SaleFromD2CConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"SaleFromD2CConsumer:It.hasNext():Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
								...
								msgsProcessed++;
								iLogger.debug("SaleFromD2CConsumer:Number of Messages processed  ="
										+ msgsProcessed);
								String msgReceived = stream.value();
								iLogger.info("SaleFromD2CConsumer:ActiveCount : " + ActiveCount
										+ " msgReceived =" + msgReceived);

								SaleFromD2CQHandler saleFromD2CObj = new SaleFromD2CQHandler();
								saleFromD2CObj.saleFromD2CInput = msgReceived;

								Callable<String> worker = saleFromD2CObj;
							...
							iLogger.debug("SaleFromD2CConsumer:Wait for Executor to terminate");
							int infiniteCounter1 = 0;
							while (!executor.isTerminated()) {
								if (infiniteCounter1 == 0) {
									iLogger.info(
											"SaleFromD2CConsumer:It.When For loops Ended :Waiting for Termination");
									infiniteCounter1++;
								}
							}

							executor.shutdownNow();
							iLogger.debug("SaleFromD2CConsumer:Executor terminated");
						}
						...
						fLogger.debug(
								"SaleFromD2CConsumer:Listener Flag is set to 0. Hence not consuming messages from Topic");
						break outerLoop;
					}
				}
			}
				...
				fLogger.fatal("SaleFromD2CConsumer:Unable to subscribe kafka consumer:Exception:"
						+ e.getMessage());
			}

			finally {
				SaleFromD2CConsumer.consumer.close();
				iLogger.info("SaleFromD2CConsumer:End on Thread Run ");
			}
		}
	}
	//CR330.en
...
/*	@SuppressWarnings({ "unchecked", "rawtypes" })
	...
	public void run() {
		...
		try {
			...
			fLogger.fatal("SaleFromD2CConsumer:Error in fetching data property File:"+e.getMessage());
		}
			...
			try{
				...
				while(numberOfThreads!=0)
				{
					...
					if(SaleFromD2CConsumer.listenerFlag==1)
					{
						...
						iLogger.info("SaleFromD2CConsumer:Initialized with ConsumerGroupId:"+groupId);

						iLogger.info("SaleFromD2CConsumer:Topic:"+topicName+":"+"Listener Flag is 1: Start Consumption from Topic"); 
							...
							for (KafkaStream stream : streams) {
								...
								while (consumerIterator.hasNext()){
									...
									if(msgsProcessed >= numberOfThreads){
										iLogger.info("SaleFromD2CConsumer:ThreadPoolCount:"+numberOfThreads+":Total Messages processed in current run:"+msgsProcessed);
										iLogger.info("SaleFromD2CConsumer:Wait for all the spawned threads to complete");
										executor.shutdown(); 
										while (!executor.isTerminated()) {   }  
										executor.shutdownNow();
										msgsProcessed=0;
									...
									iLogger.info("SaleFromD2CConsumer:msgReceived ="+msgReceived);
...
//							iLogger.debug("SaleFromD2CConsumer:Executor terminated"); 
				...
				fLogger.error("SaleFromD2CConsumer: Cause of Exception :"+e.getMessage());
			}
		}
	}*/
	//CR330.eo
