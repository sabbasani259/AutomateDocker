...
public class DealerInfoQConsumer implements Runnable {

	Thread t;
	String topicName = "DealerInfoQueue";
	...
	public DealerInfoQConsumer() {
		t = new Thread(this, "DealerInfoQConsumer");
		t.start();
	}
	...
	public void run() {
		...
		/*try {
			...
			numberOfThreads = Integer.valueOf(prop.getProperty("DealerInfoQThreadCount"));
		} catch (Exception e) {
			fLogger.fatal("DealerInfoQConsumer:Error in intializing property File:" + e.getMessage());
		}*/
		...
		if (numberOfThreads != 0) {
			try {
				...
				ThreadFactory namedThreadFactory = new ThreadFactoryBuilder()
						.setNameFormat("DealerInfoQConsumer-thread-%d").build();
				List<TopicPartition> partitionList = new LinkedList<TopicPartition>();
				TopicPartition partition = new TopicPartition(topicName, 0);
				partitionList.add(partition);
				DealerInfoQConsumer.consumer = new KafkaConsumer<>(props);
				DealerInfoQConsumer.consumer.assign(partitionList);

				outerLoop: while (numberOfThreads != 0) {
					int msgsProcessed = 0;
					int ActiveCount = 0;

					if (DealerInfoQConsumer.listenerFlag == 1) {
						ConsumerRecords<String, String> streams = DealerInfoQConsumer.consumer.poll(100);
						...
						if (streams != null && !streams.isEmpty()) {
							...
							for (ConsumerRecord<String, String> stream : streams) {
								if (DealerInfoQConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"DealerInfoQConsumer:ForKafkaStreamStart:Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"DealerInfoQConsumer:ForKafkaStreamStart:Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									infiniteCounter = 0;
									while (DealerInfoQConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"DealerInfoQConsumer:ForKafkaStreamStart:Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
									...
									iLogger.info(
											"DealerInfoQConsumer:Control Came out of Infinite Loop when Lister Falg Set to 1");
									continue outerLoop;
								}
								...
								if (executor instanceof ThreadPoolExecutor) {
									...
									iLogger.debug("DealerInfoQConsumer: ActiveCount :" + ActiveCount);
								...
								if (DealerInfoQConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"DealerInfoQConsumer:It.hasNext():Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"DealerInfoQConsumer:It.hasNext():Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									while (DealerInfoQConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info("DealerInfoQConsumer:It.hasNext():Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
								...
								msgsProcessed++;
								iLogger.debug(
										"DealerInfoQConsumer:Number of Messages processed  =" + msgsProcessed);
								String msgReceived = stream.value();
								iLogger.info("DealerInfoQConsumer:ActiveCount : " + ActiveCount + " msgReceived ="
										+ msgReceived);

								DealerInfoQHandler dealerInfoObj = new DealerInfoQHandler();
								dealerInfoObj.dealerInfoInput = msgReceived;

								Callable<String> worker = dealerInfoObj;
							...
							iLogger.debug("DealerInfoQConsumer:Wait for Executor to terminate");
							int infiniteCounter1 = 0;
							while (!executor.isTerminated()) {
								if (infiniteCounter1 == 0) {
									iLogger.info(
											"DealerInfoQConsumer:It.When For loops Ended :Waiting for Termination");
									infiniteCounter1++;
								}
							}

							executor.shutdownNow();
							iLogger.debug("DealerInfoQConsumer:Executor terminated");
						}
						...
						fLogger.debug(
								"DealerInfoQConsumer:Listener Flag is set to 0. Hence not consuming messages from Topic");
						break outerLoop;
					}
				}
			}
				...
				fLogger.fatal(
						"DealerInfoQConsumer:Unable to subscribe kafka consumer:Exception:" + e.getMessage());
			}

			finally {
				DealerInfoQConsumer.consumer.close();
				iLogger.info("DealerInfoQConsumer:End on Thread Run ");
			}
		}
	}
	...
	public void run() {
		...
		try {
			...
			fLogger.fatal("DealerInfoQConsumer:Error in fetching data property File:"+e.getMessage());
		}
			...
			try{
				...
				while(numberOfThreads!=0)
				{
					...
					if(DealerInfoQConsumer.listenerFlag==1)
					{
						...
						iLogger.info("DealerInfoQConsumer:Initialized with ConsumerGroupId:"+groupId);
	
						iLogger.info("DealerInfoQConsumer:Topic:"+topicName+":"+"Listener Flag is 1: Start Consumption from Topic"); 
							...
							for (KafkaStream stream : streams) {
								...
								while (consumerIterator.hasNext()){
									...
									if(msgsProcessed >= numberOfThreads){
										iLogger.info("DealerInfoQConsumer:ThreadPoolCount:"+numberOfThreads+":Total Messages processed in current run:"+msgsProcessed);
										iLogger.info("DealerInfoQConsumer:Wait for all the spawned threads to complete");
										executor.shutdown(); 
										while (!executor.isTerminated()) {   }  
										executor.shutdownNow();
										msgsProcessed=0;
									...
									iLogger.info("DealerInfoQConsumer:msgReceived ="+msgReceived);
	...
	//							iLogger.debug("DealerInfoQConsumer:Executor terminated"); 
				...
				fLogger.error("DealerInfoQConsumer: Cause of Exception :"+e.getMessage());
			}
		}
	}*/
}
