...
public class EngineTypeQConsumer implements Runnable {

	Thread t;
	String topicName = "EngineTypeQueue";
	...
	public EngineTypeQConsumer() {
		t = new Thread(this, "EngineTypeQConsumer");
		t.start();
	}
	...
	public void run() {
		...
		/*try {
			...
			numberOfThreads = Integer.valueOf(prop.getProperty("EngineTypeQThreadCount"));
		} catch (Exception e) {
			fLogger.fatal("EngineTypeQConsumer:Error in intializing property File:" + e.getMessage());
		}*/
		...
		if (numberOfThreads != 0) {
			try {
				...
				ThreadFactory namedThreadFactory = new ThreadFactoryBuilder()
						.setNameFormat("EngineTypeQConsumer-thread-%d").build();
				List<TopicPartition> partitionList = new LinkedList<TopicPartition>();
				TopicPartition partition = new TopicPartition(topicName, 0);
				partitionList.add(partition);
				EngineTypeQConsumer.consumer = new KafkaConsumer<>(props);
				EngineTypeQConsumer.consumer.assign(partitionList);

				outerLoop: while (numberOfThreads != 0) {
					int msgsProcessed = 0;
					int ActiveCount = 0;

					if (EngineTypeQConsumer.listenerFlag == 1) {
						ConsumerRecords<String, String> streams = EngineTypeQConsumer.consumer.poll(100);
						...
						if (streams != null && !streams.isEmpty()) {
							...
							for (ConsumerRecord<String, String> stream : streams) {
								if (EngineTypeQConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"EngineTypeQConsumer:ForKafkaStreamStart:Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"EngineTypeQConsumer:ForKafkaStreamStart:Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									infiniteCounter = 0;
									while (EngineTypeQConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info(
													"EngineTypeQConsumer:ForKafkaStreamStart:Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
									...
									iLogger.info(
											"EngineTypeQConsumer:Control Came out of Infinite Loop when Lister Falg Set to 1");
									continue outerLoop;
								}
								...
								if (executor instanceof ThreadPoolExecutor) {
									...
									iLogger.debug("EngineTypeQConsumer: ActiveCount :" + ActiveCount);
								...
								if (EngineTypeQConsumer.listenerFlag == 0) {
									int infiniteCounter = 0;
									iLogger.info(
											"EngineTypeQConsumer:It.hasNext():Listener Flag is set to 0. Hence stop consuming messages from Topic");
									executor.shutdown();
									while (!executor.isTerminated()) {
										if (infiniteCounter == 0) {
											iLogger.info("EngineTypeQConsumer:It.hasNext():Waiting for Termination");
											infiniteCounter++;
										}
										Thread.sleep(100);
									}
									...
									while (EngineTypeQConsumer.listenerFlag == 0) {
										if (infiniteCounter == 0) {
											iLogger.info("EngineTypeQConsumer:It.hasNext():Listener Flag ==0 ");
											infiniteCounter++;
										}
										Thread.sleep(1000);
									}
								...
								msgsProcessed++;
								iLogger.debug("EngineTypeQConsumer:Number of Messages processed  =" + msgsProcessed);
								String msgReceived = stream.value();
								iLogger.info("EngineTypeQConsumer:ActiveCount : " + ActiveCount + " msgReceived ="
										+ msgReceived);

								EngineTypeQHandler engineTypeObj = new EngineTypeQHandler();
								engineTypeObj.engineTypeInput = msgReceived;

								Callable<String> worker = engineTypeObj;
							...
							iLogger.debug("EngineTypeQConsumer:Wait for Executor to terminate");
							int infiniteCounter1 = 0;
							while (!executor.isTerminated()) {
								if (infiniteCounter1 == 0) {
									iLogger.info(
											"EngineTypeQConsumer:It.When For loops Ended :Waiting for Termination");
									infiniteCounter1++;
								}
							}

							executor.shutdownNow();
							iLogger.debug("EngineTypeQConsumer:Executor terminated");
						}
						...
						fLogger.debug(
								"EngineTypeQConsumer:Listener Flag is set to 0. Hence not consuming messages from Topic");
						break outerLoop;
					}
				}
			}
				...
				fLogger.fatal("EngineTypeQConsumer:Unable to subscribe kafka consumer:Exception:" + e.getMessage());
			}

			finally {
				EngineTypeQConsumer.consumer.close();
				iLogger.info("EngineTypeQConsumer:End on Thread Run ");
			}
		}
	}
	//CR330.en
	...
	public void run() {
		...
		try {
			...
			fLogger.fatal("EngineTypeQConsumer:Error in fetching data property File:"+e.getMessage());
		}
			...
			try{
				...
				while(numberOfThreads!=0)
				{
					...
					if(EngineTypeQConsumer.listenerFlag==1)
					{
						...
						iLogger.info("EngineTypeQConsumer:Initialized with ConsumerGroupId:"+groupId);
	
						iLogger.info("EngineTypeQConsumer:Topic:"+topicName+":"+"Listener Flag is 1: Start Consumption from Topic"); 
							...
							for (KafkaStream stream : streams) {
								...
								while (consumerIterator.hasNext()){
									...
									if(msgsProcessed >= numberOfThreads){
										iLogger.info("EngineTypeQConsumer:ThreadPoolCount:"+numberOfThreads+":Total Messages processed in current run:"+msgsProcessed);
										iLogger.info("EngineTypeQConsumer:Wait for all the spawned threads to complete");
										executor.shutdown(); 
										while (!executor.isTerminated()) {   }  
										executor.shutdownNow();
										msgsProcessed=0;
									...
									iLogger.info("EngineTypeQConsumer:msgReceived ="+msgReceived);
				...
				fLogger.error("EngineTypeQConsumer: Cause of Exception :"+e.getMessage());
			}
		}
	}*/
	//CR330.eo
