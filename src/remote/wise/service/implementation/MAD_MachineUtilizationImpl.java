 ...
 *
 */
public class MAD_MachineUtilizationImpl {

	public List<HashMap<String, Object>> getMachineUtilizationDetails(String loginID, List tenancyIdList,List profileCodeList,String startDate,String endDate,String accountType, String detailedView){
		...
		iLogger.info("MAD_MachineUtilizationImpl input : loginid= "+loginID+" tenancyIdList "+tenancyIdList+" profileCodeList "+profileCodeList+" startDate "+startDate+" endDate "+endDate+" accountType "+accountType+" detailedView "+detailedView);
		try{
			...
			iLogger.info("MAD_MachineUtilizationImpl : AccountCodeList "+AccountCodeList+" MDAaccountIdListAsString "+MDAaccountIdListAsString);
		...
		{
			e.printStackTrace();
			fLogger.fatal("MADashBoardDetailsRESTService:getMADMachineUtilizationDetails: "+"Error in converting the list to String :"+e.getMessage());
			return responseList;
		}
		...
		{
			e.printStackTrace();
			fLogger.fatal("MADashBoardDetailsRESTService:getMADMachineUtilizationDetails: "+"Error in intializing property File :"+e.getMessage());
			return responseList;
		}
		...
		if(date1.before(forMDA) && date2.before(forMDA)){
			...
			try{
				...
				selectQuery= "select convert(ifnull(sum(a.PeriodHMR),'0.0'),decimal(10,1)) as PeriodHMR,a.ProfileName ";
				}
				else{
					selectQuery= "select convert(ifnull(sum(a.PeriodHMR),'0.0'),decimal(10,1)) as PeriodHMR,a.AssetID as Serial_Number";
				}
				...
				iLogger.info("MADashBoardDetailsRESTService:getMADMachineUtilizationDetails: Final Query::"+finalQuery);
				...
				while(rs.next()){
					...
					respObj.put("PeriodHMR", rs.getInt("PeriodHMR"));
				...
				fLogger.fatal("MADashBoardDetailsRESTService:getMADMachineUtilizationDetails:: Exception Caught:"+e.getMessage());
			}
			finally{
				try { 
					if (rs != null)
			...
			if(tenancy_type_name!=null && !tenancy_type_name.equalsIgnoreCase(""))
			{
				...
				try{
					...
					iLogger.info("MAD_MachineUtilizationImpl:MDAIP" + connIP + " :: MDAPort" +connPort);
				}catch(Exception e){
					fLogger.fatal("MAD_MacchineUtilization : getMachineUtilizationDetails : " +
							"Exception in getting Server Details for MDA Layer from properties file: " +e);
				}
				//CR337.en
				try{
							//String url="http://10.210.196.206:26030/MoolDAReports/MADashBoardService/getMAD_MachineUtilization?accountFilter="//CR337.o
							String url="http://" + connIP +":"+connPort+"/MoolDAReports/MADashBoardService/getMAD_MachineUtilization?accountFilter=" //CR337.n
									+ accountFilter
									+ "&accountIDList="
									...
									+"&detailedView="
									+detailedView;
							//System.out.println("MDA MADashBoardService/getMAD_MachineUtilization  URL : "+url);
							iLogger.info("MDA MADashBoardService/getMAD_MachineUtilization URL : "+url);
							URL MDAUrl = new URL(url);
							 HttpURLConnection conn =(HttpURLConnection) MDAUrl.openConnection();
							    conn.setRequestMethod("GET"); 
							    conn.setRequestProperty("Accept", "application/json");
								  if (conn.getResponseCode() != 200  && conn.getResponseCode() != 204) {
									  iLogger.info("MDAReports report status: FAILURE for getMAD_MachineUtilization Service Report ::Response Code:"+conn.getResponseCode());
									  throw new RuntimeException("Failed : HTTP error code : " +conn.getResponseCode()); 
									  }
								  iLogger.info("MDAReports report status: SUCCESS for getMAD_MachineUtilization Service Report ::Response Code:"+conn.getResponseCode());								  
								  BufferedReader br = new BufferedReader(new  InputStreamReader((conn.getInputStream())));
								  
								  System.out.println("MDA MADashBoardService/getMAD_MachineUtilization Output from Server .... \n");
								  while ((MDAoutput = br.readLine()) != null) { 
									  //System.out.println("MDA AlertSummary json output "+MDAoutput); 
									  iLogger.info("MDA MADashBoardService/getMAD_MachineUtilization json output "+MDAoutput);
									  MDAresult =MDAoutput; } 
								  ...
								  if(detailedView.equalsIgnoreCase("true")){
									  ...
									  if (mapEntry.getKey().toString().contains("PeriodHMR"))
									  {
								  ...
								  //iLogger.info("MDAResponse MADashBoardService/getMAD_MachineUtilization outmap : "+MDAOutputMap);
								  conn.disconnect();
				}catch(Exception e)
				{
					e.printStackTrace();
		...
		{	e.printStackTrace();
			fLogger.fatal("Exception occured "+e.getStackTrace());
