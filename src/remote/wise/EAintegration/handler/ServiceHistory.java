...
{
	public void handleServiceHistoryDetails()
	{
		...
		{
			iLogger.info("Service History - Opening a new session");
		...
		try
		{
			Query query = session.createQuery("from JobDetails where jobCode='ServiceHistory' order by sequence");
			...
			while(itr.hasNext())
			{
				...
				ServiceHistory fileObj = new ServiceHistory();
				...
				//------------------------- Get the files related to Service History
				...
				try
				{
					...
					for(int i=0; i<processingFiles.length; i++)
					{
						...
						iLogger.info("Service History - "+"File to be processed:"+processFile.getName());
						fileObj.processServiceHistory(processFile,archivedFolderPath, jobDetailsObj.getProcess(), jobDetailsObj.getReprocessJobCode());
					}
					...
					for (int j = 0; j < inputFiles.length; j++) 
					{
						...
						if(fileMoveStatus)
							iLogger.info("Service History - "+"File "+inputFile.getName()+" is moved successfully from Input to processing folder");

						fileObj.processServiceHistory(processingFile,archivedFolderPath, jobDetailsObj.getProcess(), jobDetailsObj.getReprocessJobCode());	       
					}
				}
				...
				{
					e.printStackTrace();
					fLogger.fatal("Service History - "+"Exception :"+e);
				}
		...
		finally
		{
			...
			catch(Exception e)
			{
				...
				e.printStackTrace();
				fLogger.fatal("Service History - "+"Exception in commiting the record:"+e);
			}
	 ...
	 * @throws IOException
	 */
	public void processServiceHistory(File processingFile, String archivedFolderPath, String process, String reprocessJobCode)
	{
		Logger fLogger = FatalLoggerClass.logger;
		Logger iLogger = InfoLoggerClass.logger;

		String queuePath = "jms/queue/ServiceHistoryQueue";
		...
		if(session.getTransaction().isActive() && session.isDirty())
		{
			iLogger.info("Service History - processServiceHistory Opening a new session");
			session = HibernateUtil.getSessionFactory().openSession();
		}
		session.beginTransaction();
		...
		try
		{
			...
			if(duplicateFile==1)
			{
				...
				iLogger.info("Service History - "+"File "+processingFile.getName()+" is already processed and hence skipping the same");
				...
				if(moveStatus)
					iLogger.info("Service History - "+"File "+processingFile.getName()+" is moved successfully from processing to archived folder");
		...
		{
			fLogger.fatal("Service History - "+processingFile.getName()+": Exception :"+e);
		}
		...
		iLogger.info("Service History - processServiceHistory Closed a session");
		...
		try
		{
			...
			String processName="ServiceHistory";
			...
			while (line != null) 
			{
				...
				try{
					...
					if(msgString.length>5)
						inputObject.setJobCardDetails(msgString[5]);
					
					//DF20200131:aj20119610 Adding additional field CallTypeId
					if(msgString.length>6)
					    inputObject.setCallTypeId(msgString[6]);

					//DF20190607:IM20018382-Adding loggers to check how description field is being read
					//DF20200131:aj20119610 updated loggers to incorporate callTypeId
					iLogger.info("Input field jobCard Details for VIN "+ inputObject.getSerialNumber() + " is " + inputObject.getJobCardDetails()+" and CallTypeId : "+inputObject.getCallTypeId());
					if(inputObject.getJobCardDetails()==null){
						iLogger.info("Input field jobCard Details is set to NA as null is obtained");
						inputObject.setJobCardDetails("NA");
					}
					...
					iLogger.info(inputObject.getSerialNumber()+": Q Object Details - Service History");
					iLogger.info(inputObject.getSerialNumber()+": --------------------------");
					
					//DF20190423:IM20018382-Adding additonal field jobCardDetails
					//DF20200131:aj20119610-Adding additional field CallTypeId
					iLogger.info("Service History- processServiceHistory Input to the Q::"+inputObject.getSerialNumber()+","+inputObject.getDealerCode()+","+inputObject.getJobCardNumber()+","+"" +
							inputObject.getDbmsPartCode()+","+inputObject.getServicedDate()+"," + inputObject.getJobCardDetails()+"," +
							inputObject.getCallTypeId()+"," +
					...
					iLogger.info("Service History - processServiceHistory Before Calling Service History Q Handler");
					//DF20171212:KO369761-Changing interfaces queues to Kafka from Hornet queues.
					//String qDropStatus = queueObj.handleServiceHistoryDetails(queuePath,inputObject);
					String qDropStatus = queueObj.handleServiceHistoryDetailsToKafkaQueue("ServiceHistoryQueue",inputObject);

					iLogger.info("Service History - processServiceHistory Before Calling Service History Q Handler Status ::"+qDropStatus);
					...
					eaFileDetailsBO.storeServiceHistoryFilesData(fileName, inputObject.getJobCardNumber(),inputObject.getSerialNumber());
					...
					if(runningNumber > 1){
						...
						if(serialNumberList!=null && serialNumberList.size()==50 && serialNumberList1!=null && serialNumberList1.size()==0){
							//DF20180424 - eaFileDetailsBO variable declared globally.
							//EAFileDetailsBO eaFileDetailsBO = new EAFileDetailsBO();
			...
			if(runningNumber>1)
			{
				...
				//DF20150603 - Rajani Nagaraju - WISE going down issue - Open a new session when the getCurrentSession returns a dirty session(txns which is not yet committed exists)
				if(session==null || (session.getTransaction().isActive() && session.isDirty()))
				{
					iLogger.info("Service History - "+"Opening a new session to insert into EALogDetails");
					session = HibernateUtil.getSessionFactory().openSession();
				}
				...
				try
				{
					...
					eaLogDetails.setProcess("ServiceHistory");
				...
				iLogger.info("Service History- "+"Closed a session to insert into EALogDetails");
			}
			...
			//DF20160502 @Roopa calling ServiceHistoryQHandler queue consumer
