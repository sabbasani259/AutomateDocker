...
public class RSaleFromD2C 
{
	public void reprocessSaleFromD2C()
	{
		...
		String fileRef;
		String process = "SaleFromD2C";
		String reprocess = "RSaleFromD2C";
		String[] messageSplit = null;

		try
		{
			ErrorMessageHandler messageHandlerObj = new ErrorMessageHandler();
			List<SaleFromD2CInputContract> inputContractList = new LinkedList<SaleFromD2CInputContract>();
			...
			Query query = session.createQuery("select a.messageId, a.messageString, a.fileName, a.process," +
					...
					" a.process = b.process and a.reprocessJobCode = 'RSaleFromD2C' and a.reprocessTimeStamp <= '"+currentTimestamp+"' " +
			...
			while(itr.hasNext())
			{
				...
				SaleFromD2CInputContract inputContractObj = new SaleFromD2CInputContract();
				inputContractObj.setSellerCode(sellerCode);
				inputContractObj.setBuyerCode(buyerCode);
			...
			{
				/*SaleFromD2CClient clientObj = new SaleFromD2CClient();
        		response = clientObj.invokeSaleFromD2CService(inputContractList.get(i));*/
				//DF20180214 :@Maniratnam ## Reprocess jobs :: Adding reprocess job to kafka topic
				SaleFromD2CQHandler obj = new SaleFromD2CQHandler();
				obj.handleSaleFromD2CToKafkaQueue("SaleFromD2CQueue",inputContractList.get(i));
		...
		{
			e.printStackTrace();
			fLogger.fatal("EA ReProcessing: RSaleFromD2C: "+messgId+ " Fatal Exception :"+e);
		}
		...
		try {
			...
			String pattern = "SaleFromD2C" + "*.txt";
			...
			for (int j = 0; j < reprocessFiles.length; j++)
			{
				...
				iLogger.info("RSaleFromD2C:: from files with file name:"+reprocessFile);
				...
				while ((messageString = bufferedReader.readLine()) != null) {
					...
					SaleFromD2CQHandler obj = new SaleFromD2CQHandler();
					obj.handleSaleFromD2CToKafkaQueue("SaleFromD2CQueue",inputContractObj);
...

		{
			...
			fLogger.fatal("EA ReProcessing: RSaleFromD2C: " + messgId
					+ " Fatal Exception :" + e);
